...
STRATEGY_DB = {
    "scab": {"med": "Mancozeb", "org": "Neem Oil", "base_price": 650},
...
def analyze_sample(img_pil, heatmap, label):
    ...
    if label == "healthy":
        return {"sev": 0.0, "health": 100, "status": "Optimal", "action": "Routine Care", "cost": 150}
    ...
    if severity < 5:
        ...
        stat, act, cost = "Critical Outbreak", f"🚨 Emergency: {db['med']}", db['base_price']
...
if files:
    ...
    # Ensure model.pth is in the same folder on GitHub
    ...
    for f in files:
        ...
        with st.expander(f"Detail: {f.name} ({res['status']})"):
            ...
            c2.image(ov, use_container_width=True, caption="Heatmap")
            c3.metric("Health Score", f"{res['health']}%")
            c3.write(f"**Action:** {res['action']}")
    ...
    if summary:
        ...
        k1.metric("Avg Health", f"{df['Health'].mean():.1f}%")
        k2.metric("Cost / Acre (Proj)", f"₹{int(df['Cost'].mean() * 2.5)}")
        k3.metric("Critical Alerts", len(df[df['Severity'] > 40]))
        st.plotly_chart(px.bar(df, x="File", y="Severity", color="Diagnosis"), use_container_width=True)
